# CLAUDE.md — Mitram project context

Context for Claude Code (and human collaborators) working in `mitram/`. Read this before making changes.

## What this project is

Mitram (मित्रम्, "friend") is a Sanskrit-speaking interactive robot on **Reachy Mini Lite**, tethered to a MacBook Pro M1 Max that runs the full pipeline with **local open-source models only** (offline after model download). Wake word "mitram" → recognizes shown objects and names them in Sanskrit → converses (understands English/Kannada/Sanskrit, always replies in Sanskrit).
...
| File | What it is |
|---|---|
| `REQUIREMENTS.md` (v1.2) | Functional requirements FR-1..FR-7, memory/latency budgets, risks R1..R7, 8-week phased plan with acceptance criteria |
| `DESIGN.md` (v1.1) | Module decomposition, Strands↔Reachy integration, state machine, prompting/validation, decisions D1..D5 |
...
2. **LLM+vision = Qwen3-VL 8B Instruct via Ollama** (`qwen3-vl:8b`), chosen over Gemma 3 12B because it has **native tool calling in Ollama** (Gemma 3 doesn't) and is smaller. Revisit only via the Phase 3 Sanskrit bake-off, and record scores.
...
6. **Speech stack:** openWakeWord (custom "mitram") → Silero VAD → Whisper (mlx/whisper.cpp, en/kn; Sanskrit fine-tune experimental) → Indic Parler-TTS (Sanskrit, MPS; VITS Indic-TTS fallback).
...
The predecessor design study (edge Jetson vs AWS Bedrock Nova, translation bridge) was replaced by this design; it's preserved at git commit `40639db` (`git show 40639db:mitram/README.md`). Its key conclusion — edge Sanskrit infeasible — was revisited and reversed for the M1 Max + 2026 models (REQUIREMENTS §7).
