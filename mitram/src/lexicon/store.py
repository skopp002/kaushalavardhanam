...
the ``mitram-lexicon`` CLI. An existing row is never overwritten by generation.
"""
...
def review_cli() -> None:
    """``mitram-lexicon`` console entry point: list and verify pending names."""
    import argparse

    parser = argparse.ArgumentParser(prog="mitram-lexicon",
    ...
    print(f"\n{len(pending)} entries. Verify with: "
          f"mitram-lexicon --verify OBJECT --name नाम")
